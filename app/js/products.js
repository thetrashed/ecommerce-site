	...
	xhttp.onload = function () {
		...
		for (let i = 0; i < prod_data.length; i++) {
			...
			if (prod_data[i].stock > 0) {
				...
				var button = '<li><button onclick=addToCart()>Add To Cart</button></li>';
			} else {
			...
			doc_elem.innerHTML += div_open + '<img src="' + prod_data[i].img + '" alt="Image of item"><br/><ul><li><strong>' + prod_data[i].name + '</strong></li><li>' + price + '</li>' + button + '</ul></div>';
		}
	}
